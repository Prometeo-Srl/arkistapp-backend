...
A checklist is authored once as a **bozza**, **shared** once — which publishes it and assigns
it in the same act — and answered by the named people it reached. It does not recur, carries no
right answers, and is assigned only to individuals. See `docs/specs/0001-checklist-builder.md`,
ADR-0004 and ADR-0005.

**CHECKLIST** `id, company_id, title, description, status(draft|published|archived),
created_by_id, published_at, timestamps, deleted_at`

**CHECKLIST_SECTION** `id, uuid, checklist_id, title, position`

**CHECKLIST_QUESTION** `id, uuid, checklist_section_id, label, help_text,
type(single_choice|multi_choice|text|date|time), image_path, is_required,
allows_attachment, allows_note, position`
The prototype's cross-section drag & drop acts on `(checklist_section_id, position)`.
`image_path` is an image the **author** pins to the question; `allows_attachment` is the
**filler's** upload slot, and `allows_note` the prototype's *spazio note*.

**CHECKLIST_OPTION** `id, uuid, checklist_question_id, label, image_path, position`

The `uuid` on the three structure tables is generated by the **client** and is what
`PUT /checklists/{checklist}/structure` reconciles on, which is what makes a retried save
idempotent instead of duplicating the tree (ADR-0004). These tables are hard deleted, never
soft: that only ever happens to a bozza, because sharing freezes the structure (ADR-0005).

**CHECKLIST_ASSIGNMENT** `id, checklist_id, assignee_user_id, due_at,
status(pending|in_progress|completed|expired|cancelled), assigned_by_id, timestamps`
Named people only: a role-targeted assignment cannot answer "who still owes me this?" once the
role's membership changes. Withdrawing one cancels it rather than deleting it, so a compilazione
already handed in survives.

**CHECKLIST_SUBMISSION** `id, checklist_assignment_id, submitted_by_id, started_at,
submitted_at, status(in_progress|completed)`
Unique on `checklist_assignment_id`: the relation is a HasOne, so one submission per
assignment is enforced by the database rather than by the caller. There is no stored PDF — the
PDF is the blank questionnaire, rendered on demand by `GET /checklists/{checklist}/pdf`.

**CHECKLIST_ANSWER** `id, checklist_submission_id, checklist_question_id,
value_text, value_date, value_time, selected_option_ids_json, note_text, attachment_path`
Answers reference the question row directly, with no snapshot columns: nothing mutates after
sharing, so "which form was this" has one answer (ADR-0005).
